...
interface AppuntamentiRepository extends JpaRepository<Appuntamenti, Long> {
    ...
    @Query(value = "SELECT * FROM appuntamenti e WHERE e.ufficio = :#{#ufficio.name()} ORDER BY e.data ASC, e.ora ASC", nativeQuery = true)
    // List<Appuntamenti> findByUfficio(@Param("ufficio") String ufficio);
    List<Appuntamenti> findByUfficioOrderByDataAndOraAsc(@Param("ufficio") Uffici ufficio);
    ...
    @Modifying
    @Query(value = "DELETE FROM appuntamenti e WHERE e.data = :data AND e.ora = :ora", nativeQuery = true)
    void deleteByDataAndOra(@Param("data") LocalDate data, @Param("ora") LocalTime ora);
}